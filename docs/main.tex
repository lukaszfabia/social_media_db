...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\textwidth]{images/last_message_nosql.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\textwidth]{images/mutual_friends_nosql.png}
    \caption{Wynik kwerendy wyszukującej użytkowników ze wspólnymi znajomymi}
    \label{fig:nosql_1}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\textwidth]{images/events_with_your_friends_nosql.png}
    \caption{Wynik kwerendy wyszukującej wydarzenia, na które zapisali się znajomi użytkownika}
    \label{fig:nosql_2}
\end{figure}
...
NoSQL będą lepszym wyborem, gdy mamy do czynienia z danymi o dużym rozmiarze i dynamicznej strukturze, a także, gdy zależy nam na wydajnośći odczytu/zapisu. Przykładem tutaj mogą być, jak w naszym przypadku, media społecznościowe, ale też na przykład analizy big data etc.
...
Warto dodać, że zazwyczaj duże systemy wykorzystują różne typy baz: nie tylko dokumentowe czy relacyjne, ale też inne:


\begin{itemize}
    \item realtime database - jako kanały komunikacji
    \item bazy grafowe - do pokazania skomplikowanych relacji miedzy obiektami
    \item bazy kolumnowe - do przechowywania dużej ilości danych
\end{itemize}


Często właśnie łączy się kilka tych rozwiązań, aby mieć więcej możliwości skalowania aplikacji. Dlatego też serwisy takie jak Twiter/X korzystają z architektury mikroserwisów z uwagi na spory ruch. Dodatkowo każda baza jest odpowiednio zopytmalizowana pod jakimś kątem, mając swoje wady i zalety.
