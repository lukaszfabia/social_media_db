...
def add_article(db: Database) -> None:
    ...
    # generating hashtags
    if should_event_occur(0.9):
        hashtags = faker.words(nb=faker.random_int(
    ...
    result = db[collection.ARTICLES].insert_one(
...
def add_group(db: Database) -> None:
    ...
    for i in range(faker.random_int(min=0, max=20)):
        ...
        result_post = db[collection.POSTS].insert_one(
        ...
        for i in range(faker.random_int(min=0, max=20)):
            ...
            result_reaction = db[collection.REACTIONS].insert_one(
        ...
        for i in range(faker.random_int(min=0, max=10)):
            ...
            result_comment = db[collection.COMMENTS].insert_one(
...
def add_event(db: Database, max_members: int = 100):
    ...
    result_event = db[collection.EVENTS].insert_one(
...
def add_page(db: Database, max_likes: int = 100):
    ...
    result_page = db[collection.PAGES].insert_one(
...
def add_conversations(db: Database, max_conversations: int = 10):
    ...
    for user in users:
        for i in range(faker.random_int(min=0, max=max_conversations)):
            ...
            result_conversation = db[collection.CONVERSATIONS].insert_one(
